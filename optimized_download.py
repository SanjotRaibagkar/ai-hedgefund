...
Uses working symbols and improved screening logic.
"""
...
def get_working_symbols():
    """Get symbols that are known to work."""
    working_symbols = [
        ...
        'ADANIPORTS', 'CIPLA', 'DRREDDY', 'SHREECEM', 'BRITANNIA', 'EICHERMOT', 'HEROMOTOCO',
        'DIVISLAB', 'GRASIM', 'HINDALCO', 'UPL', 'VEDL', 'TATACONSUM', 'BPCL', 'INDUSINDBK',
        'MM', 'LT', 'APOLLOHOSP', 'BAJAJ-AUTO', 'TATACHEM', 'SBILIFE', 'HDFCLIFE', 'ICICIGI'
    ]
...
def download_all_data(symbols):
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for data in batch_data:
            if data:
                successful += 1
                print(f"  ✅ {data['symbol']}: ₹{data['close_price']}")
            else:
                failed += 1
        ...
        print(f"⏱️ Progress: {i + len(batch)}/{len(symbols)} symbols ({rate:.1f} symbols/sec)")
...
def test_screening():
    """Test the screening system with improved logic."""
    ...
    for symbol in symbols:
        try:
            ...
            if len(data) >= 3:
                ...
                # Improved screening logic
                if momentum > 2:  # 2% above average
                    signals.append({
                        'symbol': symbol,
                ...
                elif momentum < -2:  # 2% below average
                    signals.append({
                        'symbol': symbol,
                        'current_price': current_price,
                        'avg_price': avg_price,
...
def main():
    ...
    print(f"📊 Using {len(symbols)} working symbols")
