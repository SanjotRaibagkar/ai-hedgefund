#!/usr/bin/env python3
"""
Check Comprehensive DuckDB Database
"""
...
def check_comprehensive_duckdb():
    """Check comprehensive DuckDB database."""
    conn = duckdb.connect('data/comprehensive_equity.duckdb')
    
    print("🗄️ COMPREHENSIVE DUCKDB DATABASE ANALYSIS")
    print("=" * 60)
    ...
    print("\n" + "="*80)
    print("📊 DETAILED COMPREHENSIVE PRICE_DATA TABLE STATISTICS")
    print("="*80)
    ...
    for _, row in symbol_counts.iterrows():
        print(f"  {row['symbol']:<12} {row['record_count']:>6} records  ({row['first_date']} to {row['last_date']})")
    ...
    for _, row in most_data.iterrows():
        print(f"  {row['symbol']:<12} {row['record_count']:>6} records")
    
    conn.close()

if __name__ == "__main__":
    check_comprehensive_duckdb()
