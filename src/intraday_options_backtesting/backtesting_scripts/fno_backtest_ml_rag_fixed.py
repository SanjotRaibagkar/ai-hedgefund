#!/usr/bin/env python3
"""
FNO Comprehensive Backtesting for ML+RAG System - FIXED VERSION
Tests predictions vs actual outcomes using only fno_bhav_copy table
Fixed to calculate realistic returns
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fno_backtest_ml_rag_fixed.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class FNOBacktestMLRAGFixed:
    """FNO backtesting system using only fno_bhav_copy table with fixed return calculation"""
    ...
    def simulate_ml_prediction(self, features: Dict) -> float:
        """Simulate ML prediction based on FNO features"""
        try:
            # Simple ML-like prediction based on FNO features
            base_prob = 0.5
            
            # Adjust based on daily return
            ...
            if pcr > 1.5:  # High put activity (bearish)
                base_prob -= 0.08
            elif pcr < 0.7:  # High call activity (bullish)
                base_prob += 0.08
            
            # Adjust based on OI change
            oi_change = features.get('oi_change', 0)
            if oi_change > 1000000:  # High OI buildup
                base_prob += 0.05
    ...
    def run_single_test(self, symbol: str, test_date: str, horizon: str) -> Optional[Dict]:
        ...
        try:
            ...
            ml_prediction = self.simulate_ml_prediction(features)
    ...
    def run_comprehensive_backtest(self, 
        ...
        logger.info(f"Starting FNO comprehensive backtest (FIXED) from {start_date} to {end_date}")
    ...
    def save_results(self, results_df: pd.DataFrame, filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"fno_comprehensive_backtest_results_FIXED_{timestamp}.csv"
        ...
        with open(summary_filename, 'w') as f:
            f.write("FNO COMPREHENSIVE ML+RAG BACKTEST SUMMARY (FIXED)\n")
            f.write("=" * 60 + "\n\n")
...
def main():
    ...
    logger.info("Starting FNO Comprehensive ML+RAG Backtesting (FIXED VERSION)")
    
    # Initialize backtester
    backtester = FNOBacktestMLRAGFixed()
    ...
    try:
        ...
        if not results_df.empty:
            ...
            print(f"\n{'='*60}")
            print("FNO COMPREHENSIVE ML+RAG BACKTEST RESULTS (FIXED)")
            print(f"{'='*60}")
