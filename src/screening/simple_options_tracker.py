#!/usr/bin/env python3
"""
Simple Options Tracker - ATM ± 2 Strikes OI-Based Strategy
Uses working NSE API methods for options data.
"""
...
class SimpleOptionsTracker:
    """Simple Options Tracker using working NSE API methods."""
    
    def __init__(self):
        """Initialize Simple Options Tracker."""
    ...
    def run_analysis(self, index: str = 'NIFTY') -> Dict[str, Any]:
        ...
        try:
            # 1. Get options chain (this includes underlyingValue for spot price)
            options_data = self._get_options_chain(index)
            if options_data is None or options_data.empty:
                return {'error': f'Could not get options data for {index}'}
            
            # 2. Get current spot price from options data
            current_price = self._get_spot_price_from_options(options_data)
    ...
    def _get_options_chain(self, index: str) -> Optional[pd.DataFrame]:
        ...
        try:
            # Try get_live_option_chain first (more real-time)
    ...
    def _get_spot_price_from_options(self, options_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            self.logger.error(f"Error getting spot price from options: {e}")
            return None
    ...
    def _find_atm_strike(self, current_price: float, options_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            if strike_col not in options_data.columns:
                self.logger.error(f"No strike price column found: {list(options_data.columns)}")
                return None
    ...
    def _analyze_oi_patterns(self, options_data: pd.DataFrame, atm_strike: float, current_price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'Strike_Price' in options_data.columns:
                # Live options chain format
                return self._analyze_live_options_format(options_data, atm_strike, current_price)
            else:
                # Regular options chain format
                return self._analyze_regular_options_format(options_data, atm_strike, current_price)
    ...
    def _analyze_live_options_format(self, options_data: pd.DataFrame, atm_strike: float, current_price: float) -> Dict[str, Any]:
        """Analyze OI patterns for live options chain format."""
        try:
            # Get ATM ± 2 strikes
            ...
            self.logger.error(f"Error analyzing live options format: {e}")
            return {'pcr': 0, 'atm_call_oi': 0, 'atm_put_oi': 0, 'atm_call_oi_change': 0, 'atm_put_oi_change': 0, 'iv_regime': 'UNKNOWN'}
    
    def _analyze_regular_options_format(self, options_data: pd.DataFrame, atm_strike: float, current_price: float) -> Dict[str, Any]:
        """Analyze OI patterns for regular options chain format."""
        try:
            # Get ATM ± 2 strikes
            ...
            for strike in strikes_to_analyze:
                ...
                if not call_data.empty:
                    call_oi = int(call_data['openInterest'].iloc[0]) if 'openInterest' in call_data.columns else 0
                    total_call_oi += call_oi
                    
                    if strike == atm_strike:
                        atm_call_oi = call_oi
                        atm_call_oi_change = int(call_data['changeinOpenInterest'].iloc[0]) if 'changeinOpenInterest' in call_data.columns else 0
                
                # Put data
                put_data = strike_data[strike_data['instrumentType'] == 'PE']
                if not put_data.empty:
                    put_oi = int(put_data['openInterest'].iloc[0]) if 'openInterest' in put_data.columns else 0
                    total_put_oi += put_oi
                    
                    if strike == atm_strike:
                        atm_put_oi = put_oi
                        atm_put_oi_change = int(put_data['changeinOpenInterest'].iloc[0]) if 'changeinOpenInterest' in put_data.columns else 0
            ...
            self.logger.error(f"Error analyzing regular options format: {e}")
            return {'pcr': 0, 'atm_call_oi': 0, 'atm_put_oi': 0, 'atm_call_oi_change': 0, 'atm_put_oi_change': 0, 'iv_regime': 'UNKNOWN'}
...
# Global instance
simple_options_tracker = SimpleOptionsTracker()
